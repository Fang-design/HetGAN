...
#         # adj_data = adj_data.drop(['MODZCTA'], axis=1)
...
def create_dataset(dataset, windows):
    dataX, dataY = [], []
    for i in range(len(dataset) - windows):
        a = dataset[i:(i + windows)]
        dataX.append(a)
...
def load_datasets(data,windows, embeding_size,data_type='flow'):
    final_data_x = []
    final_data_y = []

    for index, value in enumerate(data):
        #dataset = normalization(value)
        dataset_X, dataset_Y = create_dataset(value, windows)
        final_data_x.append(dataset_X)
        final_data_y.append(dataset_Y)
    final_data_x = np.array(final_data_x)  # 439 * (365 - 7) * 7
    final_data_y = np.array(final_data_y)  # 439 * (365 - 7)
